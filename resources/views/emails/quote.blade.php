...
<h1>Új ajánlatkérés érkezett a(z) {{ $domain }} oldalról</h1>

@foreach ($fields as $label => $value)
    @continue($value === null || $value === '')
    <p><strong>{{ $label }}:</strong>
        @if (in_array($label, ['Referencia oldalak', 'Kulcsfunkciók', 'Szükséges integrációk', 'Marketing célok', 'Jogi / megfelelőségi elvárások']) || (is_string($value) && str_contains($value, "\n")))
            <br>{!! nl2br(e($value)) !!}
        @else
            {{ $value }}
        @endif
